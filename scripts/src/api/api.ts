...
const getTransistorData = async (idx: number | string): Promise<TransistorData | null> => {
  try {
...
interface TransistorRarityData {
  metricName: string
  attrName: string
  rate: number
  count: number
...
const getTransistorRarity = (rarityList: Array<TransistorRarityData>, metricName: string) : TransistorRarityData|null => {
  for (let i = 0; i < rarityList.length; i++) {
    const r = rarityList[i]
